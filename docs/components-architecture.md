...
- общий экран фильтрации по дате, локации, виду спорта и тегам
- используется в desktop-карточках и mobile bottom sheet

Как работает:

- `embedded` переключает режим встраивания в bottom sheet
- `Calendar` отображает выбор диапазона дат
- `sportSearch` и `locationSearch` фильтруют списки чекбоксов
- `toggleSport()`, `toggleLocation()` и `toggleTag()` переключают локальные значения
- `resetFilter()` сбрасывает локальное состояние фильтра
- `close` эмитится по клику на кнопку закрытия
...
- тестовый экран для проверки комбинации фильтров, календаря и чекбоксов
