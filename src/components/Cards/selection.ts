...
export function resolveSelection(marker: Marker, sourceMarkers: Marker[]): SelectionTarget {
  // Сначала пытаемся понять, не является ли кликнутый элемент самим проектом.
  const project = sourceMarkers.find(item => item.id === marker.id)
  ...
  for (const item of sourceMarkers) {
