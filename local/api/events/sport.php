...
const SPORT_SECTION_CODE = 'sport';
const SPORT_SECTION_NAME = 'Спорт';
...
function findSportSectionId(): ?int
{
...
function normalizeSportTypes($property): array
{
    $values = is_array($property) && isListArray($property) ? $property : [$property];
    $sportTypes = [];

    foreach ($values as $item) {
        $title = trim((string) (is_array($item) ? ($item['VALUE_ENUM'] ?? $item['VALUE'] ?? '') : $item));
    ...
    return array_values(array_unique($sportTypes));
}

function mapElementToSportMarker(array $element): array
{
    ...
    $sportTypes = normalizeSportTypes($properties['PROP4'] ?? []);
...
$sectionId = findSportSectionId();
if ($sectionId === null) {
    respondJson(['error' => 'sport section not found'], 404);
}

$items = [];
$elements = CIBlockElement::GetList(
    ['ACTIVE_FROM' => 'DESC', 'SORT' => 'ASC', 'ID' => 'ASC'],
    [
        'IBLOCK_ID' => EVENTS_IBLOCK_ID,
...
while ($element = $elements->Fetch()) {
    $items[] = mapElementToSportMarker($element);
}
